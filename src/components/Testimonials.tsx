        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {testimonials.map((t, i) => (
            <motion.div
              ...
              className={`relative rounded-2xl border border-border bg-card p-6 hover:shadow-md transition-shadow`}
            >
              <Quote size={28} className="text-primary/15 absolute top-5 right-5" />

              <div className="flex items-center gap-3 mb-4">
                <div className={`w-11 h-11 rounded-full bg-gradient-to-br ${t.gradient} flex items-center justify-center text-sm font-bold text-foreground`}>
                  {t.avatar}
                </div>
                <div>
