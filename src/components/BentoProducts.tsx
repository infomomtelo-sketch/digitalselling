...
const BentoProducts = () => {
  ...
  return (
    <section id="products" className="py-24 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      <div className="absolute top-0 right-1/4 w-[500px] h-[500px] rounded-full bg-primary opacity-[0.04] blur-[150px]" />
        ...
        {filteredProducts.length === 0 ? (
          ...
          >
            {filteredProducts.map((product) => (
              ...
              <motion.div
                variants={item}
                className={`${product.span} group relative rounded-2xl border border-border bg-card p-6 hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5 transition-all duration-300 cursor-pointer overflow-hidden h-full`}
              >
